...
const SAMPLE_PLANTS = [
  { name: 'Aloe vera', type: 'Herb', emoji: '🌱' },
  { name: 'Mango', type: 'Fruit', emoji: '🥭' },
  { name: 'Tomato', type: 'Vegetable', emoji: '🍅' },
  { name: 'Hibiscus', type: 'Flower', emoji: '🌺' },
  { name: 'Ginger', type: 'Herb', emoji: '🫚' },
  { name: 'Basil', type: 'Herb', emoji: '🌿' },
]
...
function App() {
  ...
  const filteredPlants = useMemo(() => {
    ...
    return SAMPLE_PLANTS.filter(
      (plant) => plant.name.toLowerCase().includes(term) || plant.type.toLowerCase().includes(term),
    )
  }, [searchTerm])
      ...
      <main>
        {activeTab === 'home' ? (
          <>
            ...
            <section className="fv-section">
              <SectionHeading eyebrow="DISCOVER" title="Popular specimens" action="Explore" onAction={() => setActiveTab('explore')} />
              <div className="fv-specimen-row">
                {SAMPLE_PLANTS.slice(0, 4).map((plant) => (
                  <button type="button" className="fv-specimen-card" key={plant.name} onClick={() => setActiveTab('explore')}>
                    <span>{plant.emoji}</span><strong>{plant.name}</strong><small>{plant.type}</small>
                  </button>
                ))}
              </div>
            </section>
          ...
          <section className="fv-page-section">
            ...
            <div className="fv-specimen-grid">
              {filteredPlants.map((plant) => (
                <button type="button" className="fv-large-specimen" key={plant.name} onClick={openCamera}>
                  <span className="fv-large-emoji">{plant.emoji}</span>
                  <strong>{plant.name}</strong>
                  <small>{plant.type}</small>
                  <span className="fv-identify-link">Identify with AI<ArrowRight size={15} /></span>
                </button>
              ))}
            </div>
